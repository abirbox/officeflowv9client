...
import { Plus, Filter, X, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, MoreVertical, Columns as ColumnsIcon, Download, GripVertical, Eye, EyeOff, Upload } from 'lucide-react';
...
const DispatchSchedulePage = ({ todayOnly = false }) => {
  ...
  const emptySiteForm = { post_pin: '', name: '', location: '', city: '', notes: '' };
  ...
  const submit = async () => {
    // client-side required check (work_order_number is optional)
    for (const k of ['date', 'shift_type', 'start_time', 'end_time', 'client_id', 'vendor_id', 'post_site_id', 'officer_id']) {
      if (!form[k]) { toast.error(`${k.replace(/_/g, ' ')} is required`); return; }
    }
    try {
      if (editing) {
